...
assert(api.G.bots.filter((b) => b.eventSpawn).length <= 24);
for (let i = 0; i < 8; i++) api.updateEvents(1);
assert(api.G.bots.filter((b) => b.eventSpawn).length > 36);
assert(api.G.bots.filter((b) => b.eventSpawn).length <= 72);
...
assert(api.G.bots.filter(b => b.swarming).length <= 18, 'Only a bounded local group surrounds the player');
